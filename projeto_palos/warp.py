...
retval, thresholded = cv2.threshold(saturation, 0, 255, cv2.THRESH_OTSU)

# morphological operations
thresholded_open = cv2.morphologyEx(thresholded, cv2.MORPH_OPEN, kernel)

thresholded_close = cv2.morphologyEx(thresholded_open, cv2.MORPH_CLOSE, kernel)

'''
cv2.imshow("thresholded", thresholded_close)
cv2.waitKey() 
'''
...
# draw a contour
cv2.drawContours(flat_object_resized_copy, [our_cnt], -1, (0,255,0), 2)
warped = for_point_warp(our_cnt/ratio, flat_object)
warped = resize(warped, height=600)
